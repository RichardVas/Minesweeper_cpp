...
/*ITT FOLYTASD*/
//void flood_fill(std::vector<std::vector<int>>& grid, std::vector<std::vector<int>>& sgrid, int i, int j)
//{
//	/*ez a fv találja meg a szomszédos üres cellákat a kapott coordok segítségével*/
//	
//	int n = 0;
//
//
//	std::vector<flood>cells;
//	if (grid[i][j] == 0) 
//	{
//		cells.push_back(flood(i, j));
//
//	}
//			if (grid[i + 1][j] == 9) n++;
//			if (grid[i][j + 1] == 9) n++;
...
/*el kell menteni a grideket külön- külön, vagy egy összetett algortimus az olvasáshoz*/
...
auto gui_lambda= []() {
	...
	while (app.isOpen())
	{
		...
		if (!isgameover) {
			score = score_lambda();//fölösleges átadni, csak akkor ha még nincs vége a játéknak
		}
